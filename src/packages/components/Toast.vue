<template>
  <li
    ...
    "
    data-sonner-toast=""
    :data-rich-colors="String(toast.richColors != null ? toast.richColors : defaultRichColors)"
    ...
    :data-swiping="String(swiping)"
    :data-dismissible="String(dismissible)"
    :data-type="toastType"
    :data-invert="String(toast.invert || invert)"
    :data-swipe-out="String(swipeOut)"
    :data-swipe-direction="swipeOutDirection"
    :data-expanded="String(Boolean(expanded || (expandByDefault && mounted)))"
  ...
  >
    ...
    <template v-if="toast.component">
      <component
        ...
        :isPaused="expanded || interacting || isDocumentHidden"
      />
    </template>
        ...
        <template v-if="toast.description">
          ...
          >
            <template v-if="isStringOfDescription">
              <component :is="toast.description" v-bind="getProps()" />
            </template>
            <template v-else>
              {{ toast.description }}
            </template>
        ...
        <button
          ...
          @click="
            (event) => {
              ...
              toast.cancel.onClick && toast.cancel.onClick(event);
              deleteToast();
            }
          "
        >
        ...
        <button
          ...
          @click="
            (event) => {
              ...
              toast.action.onClick && toast.action.onClick(event);
...
import { cn, getDefaultSwipeDirections, useIsDocumentHidden } from '../hooks';
...
function getProps() {
  return props.toast.componentProps;
}
...
onMounted(() => {
  ...
  const {height} = toastRef.value.getBoundingClientRect()
  initialHeight.value = height

  const newHeights = [
    { toastId: props.toast.id, height, position: props.toast.position || props.position },
    ...props.heights
  ]
