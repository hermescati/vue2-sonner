...
import SuccessIcon from '@/packages/assets/SuccessIcon.vue'
...
const actualTheme = ref(
  ...
  props.theme !== 'system' ? props.theme : typeof window !== 'undefined' ? window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light' : 'light'
)
...
function updateHeight(h: HeightT) {
  ...
  if (index !== -1) {
    heights.value.splice(index, 1, h)
  } else {
